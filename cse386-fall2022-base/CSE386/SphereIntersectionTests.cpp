...
bool cameraSeesSphere(const RaytracingCamera& cam, const ISphere& sphere) {
    ...
    for (int x = 0; x < cam.getNX(); x++) {
        for (int y = 0; y < cam.getNY(); y++) {
            if (rayIntersectsSphere(cam.getRay(x, y), sphere)) return true;
        }
    }
...
int whichSphere(const Ray& ray, const ISphere& sphere1, const ISphere& sphere2) {
    HitRecord hits1[2],hits2[2];
    int hitCount1 = sphere1.findIntersections(ray, hits1);
    int hitCount2 = sphere2.findIntersections(ray, hits2);

    if (hitCount1 == 0 && hitCount2 == 0) return 0;
    double tcomp = hits1[0].t - hits2[0].t;

    
    if (approximatelyZero(tcomp)) return 3;
    if (tcomp > 0) return 2;
    if (tcomp < 0) return 1;
}

int old_main3(int argc, char* argv[]) {
    ...
    ISphere s1(dvec3(20, 0, 0), 5.0),
        ...
        s3(dvec3(-8, 0, 0), 5.0),
        s4(dvec3(-20, 0, 0), 5.0);
    std::cout << whichSphere(ray, s1, s2) << std::endl;
    std::cout << whichSphere(ray, s1, s1) << std::endl;
    std::cout << whichSphere(ray, s1, s3) << std::endl;
    std::cout << whichSphere(ray, s3, s4) << std::endl;
